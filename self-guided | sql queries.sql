...
-- query below yields different results than in the assignemnt, but the amounts are lower too
...
-- In the order table, which account_ids were responsible for orders between order_id 29540 and order_id 29560 
-- (inclusive)?
...
select * from client;
select district_id, count(client_id) as total_per_district from client
...
select date, count(loan_id) as total_loans_per_day 
from loan
where date < 930907
group by date
order by date desc;
...
from loan
where substring(date, 1, 4) = 9712
group by date, duration
order by date asc, duration asc;


-- In the trans table, for account_id 396, sum the amount of transactions for each type 
-- (VYDAJ = Outgoing, PRIJEM = Incoming). Your output should have the account_id, the type and the sum of 
...
end) as type, floor(sum(amount)) as total_amount
from trans
where account_id = 396
group by type;
